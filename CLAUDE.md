...
اللغة الافتراضية **عربي (RTL)** مع تبديل للإنجليزي، ووضع فاتح/داكن/حسب النظام.
...
ورمز `text-static-white` الذي لا ينقلب في الوضع الداكن.
...
خمسة أدوار تشغيلية فوق أدوار Shelf الأصلية:

| الدور        | enum           | الدور في سير العمل                       |
| ------------ | -------------- | ---------------------------------------- |
| تقنية المعلومات | `OWNER`        | صلاحيات كاملة                            |
| المستودعات   | `WAREHOUSE`    | يضيف الأصل، يعتمده، يبتّ في طلبات الموظفين |
| المالية      | `FINANCE`      | يعدّل بيانات الأصل والترميز — **لا يضيف ولا يعتمد** |
| المخزون      | `INVENTORY`    | يراقب، ويعلّق الطلبات للمراجعة — **لا يقرّر** |
| الموظف       | `SELF_SERVICE` / `BASE` | يطلب لنفسه فقط                  |
...
- `approve` — نقل عنصر إلى الأمام في سير العمل (اعتماد أصل، قبول طلب).
...
| --------------------- | ---------------------------------------------------------- | ------- |
| استلام الأصل          | `Asset.lifecycleStage`: `PENDING` → `READY` بيد المستودعات | [epda-asset-intake-workflow.md](./apps/docs/epda-asset-intake-workflow.md) |
| طلبات الموظفين        | `Booking.approvalState` + تعليق المخزون للمراجعة            | [epda-booking-request-workflow.md](./apps/docs/epda-booking-request-workflow.md) |

**النمط المشترك بينهما — اتبعه في أي سير عمل جديد:** الحالة تُضاف كحقل مستقل
...
- **خدماتي** (الجميع): الأصول المتاحة · الأصول في عهدتي · الحجوزات
- **المخزون** (`!isScopedToOwnRecords`): الأصول · المجموعات · المواقع · التصنيفات · الوسوم
- **العمليات**: الطلبات · الجرد · التذكيرات · التقارير
- **المنظمة**: الفريق · إعدادات مساحة العمل

فهرسا `/assets` و `/kits` **مخفيان عن الموظف** — سطحا تصفّح للمخزون كله لا
يستطيع أن يفعل فيهما شيئاً، وقد غطّتهما «الأصول المتاحة». المساران ما زالا
...
Vite يحتفظ بنسخة مُحسَّنة مسبقاً من `@prisma/client` فتبقى القيم القديمة
(`undefined` للتعدادات الجديدة) تُقدَّم حتى بعد التوليد.
