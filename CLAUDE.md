...
- **جداول `Booking*` باقية في `schema.prisma`** — لكن **ليس صحيحاً أن لا كود
  يقرأها**: ٣١ ملفاً ما زال يستعلم `bookingAssets` / `BookingStatus` (انظر
  البند التالي). لا تحذف قيمة `CHECKED_OUT` من `AssetStatus` مع الترحيل
  (تُفحص `switch` شاملاً في عشرات المواضع).
...
- **`availableToBook` باقٍ ولم يُلمس** — و**ليس بلا مستهلك** خلافاً لما كان
  ...
  و**تصدير CSV** (`csv.server.ts`، `import-ready-export.server.ts`) و**شارة
  المجموعة** (`kit-status-badge.tsx`) و**عمود في الفهرس المتقدّم**
  (`advanced-asset-columns.tsx`). أي أن شارة «غير متاح للحجز» ما زالت **ظاهرة
...
٣١ ملفاً ما زالت فيه استعلامات حجز حيّة تغذّي **شارة حالة الصنف ونافذة تفصيل
الكمية** — أبرزها `components/assets/asset-status-badge/quantity-data.ts`،
و`api+/assets.$assetId.quantity-breakdown.ts`، و`modules/{asset,kit,location,
