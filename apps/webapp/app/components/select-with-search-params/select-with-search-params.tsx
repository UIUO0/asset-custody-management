...
export default function SelectWithSearchParams({
  ...
  return (
    <Select
      ...
      disabled={disabled}
    >
      <SelectTrigger className="px-3.5 py-2 text-start text-base text-gray-500 md:mt-0 md:max-w-fit">
      ...
      >
        {items.map((item) => (
          <SelectItemComponent
            key={item.value}
            value={item.value}
            className="rounded-none border-b border-gray-200 px-6 py-4 pe-1.5 text-gray-700 hover:bg-gray-50"
          >
            <span className="me-4 text-sm">{item.label}</span>
          </SelectItemComponent>
        ))}
      </SelectContent>
    </Select>
