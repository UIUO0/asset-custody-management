                ...
                >
                  <TabsList>
                    <TabsTrigger value="scanner" disabled={isLoading || paused}>
                      <ScanQrCode className="me-2 size-5" /> Scanner
                    </TabsTrigger>
                    <TabsTrigger value="camera" disabled={isLoading || paused}>
                      <CameraIcon className="me-2 size-5" /> Camera
                    </TabsTrigger>
                  </TabsList>
    ...
    >
      ...
      <div className="relative mx-auto mb-4 size-16">
        <div className="absolute inset-0 flex items-center justify-center">
          <QrCode className="size-8  text-static-white/90" />
        </div>
        <div className="animate-ping absolute inset-0 rounded-full border-4 text-static-white/80 opacity-30"></div>
      </div>
      <div className="relative flex items-center gap-3">
        <Input
          ref={inputRef}
          className="items-center [&_.inner-label]:font-normal [&_.inner-label]:text-static-white"
          inputClassName="scanner-mode-input max-w-[460px] min-w-[360px] pe-[56px]"
          disabled={paused}
          name="code"
          label={
            paused
...
function CameraMode({
  ...
  return (
    <>
      ...
      {error && error !== "" && (
        <InfoOverlay>
          <div className="mx-auto mb-6 flex size-32 items-center justify-center rounded-lg border-2 border-dashed border-static-white/30">
...
function InfoOverlay({ children }: { children: ReactNode }) {
  return (
    <div className="info-overlay absolute inset-0 z-20 flex items-center justify-center bg-slate-800 px-5">
      <RadialBg />
      <div className="z-10 text-center text-static-white">{children}</div>
    </div>
  );
}
