  ...
  const fieldTypeToCompMap: {
    ...
    DATE: (field) => (
      <div className="flex w-full items-end">
        ...
        {dateObj[field.id] ? (
          <Button
            className="ms-2 h-[42px] sm:h-full"
            icon="x"
            variant="secondary"
            type="button"
            onClick={() => {
      ...
      <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
        <PopoverTrigger asChild>
          <Button
            type="button"
            variant="secondary"
            className="w-full justify-start truncate whitespace-nowrap font-normal [&_span]:w-full [&_span]:max-w-full [&_span]:truncate [&_span]:text-start"
            ref={triggerRef}
          >
