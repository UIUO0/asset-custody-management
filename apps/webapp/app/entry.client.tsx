...
/**
 * Resolves the locale the server rendered with, so hydration matches the SSR
 * markup exactly. `<html lang>` is the source of truth (the server sets it from
 * the same cookie), with the cookie as a fallback for the rare case where the
 * attribute is missing.
 *
 * @returns The locale to initialise the client i18next instance with
 */
function getInitialLocale() {
  ...
  return parseLocale(fromCookie) ?? DEFAULT_LOCALE;
}
