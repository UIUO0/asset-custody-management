/**
 * Custody / bookings visibility helpers.
 *
 * Isomorphic on purpose. These used to live in a `.client.ts` module, which
 * React Router's vite plugin stubs to `undefined` in the SERVER bundle — so
 * every loader and server-rendered component that called them was one render
 * away from a bare 500. The logic is pure (roles + org settings in, boolean
 * out) with no browser or Node dependency, so there was never a reason for the
 * suffix.
 *
 * `custody-and-bookings-permissions.validator.ts` re-exports this for
 * backwards compatibility.
 *
 * @see {@link file://./permission.validator.ts} same fix, same reasoning
 */
