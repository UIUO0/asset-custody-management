...
/** Columns needed to serialize a booking. */
export const API_BOOKING_SELECT = {
  id: true,
  name: true,
  status: true,
  approvalState: true,
  ...
  createdAt: true,
  updatedAt: true,
  custodianUser: { select: { firstName: true, lastName: true, email: true } },
  custodianTeamMember: { select: { name: true } },
  creator: { select: { firstName: true, lastName: true, email: true } },
  // The relation is `bookingAssets` (a pivot), not `assets`. The public shape
  // flattens it to a plain `assetCount` so the pivot's existence stays internal.
...
export type ApiBooking = {
  ...
  name: string;
  status: string;
  /** EPDA request workflow: PENDING / APPROVED / REJECTED. */
...
/** Joins a person's name parts, falling back to their email. */
function personName(
  person: {
...
/**
 * Shapes a booking row for the public API.
 *
 * @param booking - A row selected with {@link API_BOOKING_SELECT}
 * @returns The public booking shape
 */
export function serializeBooking(booking: {
  ...
  return {
    ...
    // A booking's custodian is either a registered user or a non-registered
    // team member; the public shape collapses both to a display name.
    custodian:
      personName(booking.custodianUser) ??
      booking.custodianTeamMember?.name ??
      null,
    createdBy: personName(booking.creator),
